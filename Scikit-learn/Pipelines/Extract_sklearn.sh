...
if grep -wq "Scikit-learn script" $file && grep -wq "Elapsed time:" $file;then
  ...
  if [[ $choice == "r" ]];then
    if grep -wq "Mean absolute error" $file;then
	  ...
	  echo "    --> The $file file doesn't contain scikit-learn regression outputs! Skipping..."
    ...
    if grep -wq "Classification" $file;then 
	  ...
	  echo "    --> The $file file doesn't contain scikit-learn classification outputs! Skipping..."
      printf "%-8s %-50s %15s %15s %15s %15s %15s\n" "X" ${file%.$inp_extension} "N/A" "N/A" "N/A" "N/A" "N/A" >> $initial_path/Extract_sklearn_results.txt
      let file_counter--
	fi
  fi

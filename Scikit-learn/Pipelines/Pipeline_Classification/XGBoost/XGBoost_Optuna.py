...
model_name = "XGBClassifier"  #  "XGBRegressor" or "XGBClassifier"
model_params = {'random_state':0, 'objective':'multi:softmax', 'num_class':2, 'eval_metric':'logloss', 'use_label_encoder':None} 
...
pipe_params = {}
pipe_params['xgbc__learning_rate'] = optuna.distributions.LogUniformDistribution(1e-4,1e0)
pipe_params['xgbc__min_child_weight'] = optuna.distributions.LogUniformDistribution(1e-2,1e2)
pipe_params['xgbc__max_depth'] = optuna.distributions.IntUniformDistribution(2,20,1)
pipe_params['xgbc__gamma'] = optuna.distributions.LogUniformDistribution(1e-2,1e1)
pipe_params['xgbc__max_delta_step'] = optuna.distributions.CategoricalDistribution([0,1])
pipe_params['xgbc__subsample'] = optuna.distributions.UniformDistribution(0.5,1)
pipe_params['xgbc__colsample_bytree'] = optuna.distributions.CategoricalDistribution([0.3,0.5,0.8,1.0])
pipe_params['xgbc__reg_lambda'] = optuna.distributions.LogUniformDistribution(1e-4,1e0)  # L2 regularization 

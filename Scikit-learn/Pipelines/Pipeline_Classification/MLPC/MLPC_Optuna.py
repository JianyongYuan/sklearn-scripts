...
model_name = "MLPClassifier"  # "MLPRegressor" and "MLPClassifier"
model_params = {'random_state':0, 'max_iter':800, 'solver':'adam', 'early_stopping':True}
...
columns_list_ss = selected_features
...
column_trans = DataFrameMapper(
                     [
                      (columns_list_ss, StandardScaler())],
                        None
                        )
...
# tuple_combinations = [(i,i,i) for i in range(5,40,1)] +\
                     # [(i,i,i,i) for i in range(5,40,1)] +\
                     # [(i,i,i,i,i) for i in range(5,40,1)] +\
                     # [(i,i,i,i,i,i) for i in range(5,40,1)] +\
                     # [(i,i,i,i,i,i,i) for i in range(5,40,1)] +\
                     # [(i,i,i,i,i,i,i,i) for i in range(5,40,1)] 
tuple_combinations = [tuple([i for j in range(k)]) for k in range(3,9) for i in range(5,40,1)]  

pipe_params = {}
pipe_params['mlpc__activation'] = optuna.distributions.CategoricalDistribution(['identity', 'logistic', 'tanh', 'relu'])
pipe_params['mlpc__alpha'] = optuna.distributions.LogUniformDistribution(1e-3,1e3)
pipe_params['mlpc__hidden_layer_sizes'] = optuna.distributions.CategoricalDistribution(tuple_combinations)
pipe_params['mlpc__learning_rate_init'] = optuna.distributions.LogUniformDistribution(1e-6,1e-1)

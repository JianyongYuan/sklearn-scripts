...
model_name = "XGBRegressor"  #  "XGBRegressor" or "XGBClassifier"
model_params = {'random_state':0, 'objective':'reg:squarederror', 'eval_metric':'rmse'} 
scoring_metrics = "neg_mean_squared_error" #   https://scikit-learn.org/stable/modules/model_evaluation.html#scoring-parameter
...
pipe_params = {}
pipe_params['xgbr__learning_rate'] = optuna.distributions.LogUniformDistribution(1e-4,1e0)
pipe_params['xgbr__min_child_weight'] = optuna.distributions.LogUniformDistribution(1e-2,1e2)
pipe_params['xgbr__max_depth'] = optuna.distributions.IntUniformDistribution(2,20,1)
pipe_params['xgbr__gamma'] = optuna.distributions.LogUniformDistribution(1e-2,1e1)
pipe_params['xgbr__max_delta_step'] = optuna.distributions.CategoricalDistribution([0,1])
pipe_params['xgbr__subsample'] = optuna.distributions.UniformDistribution(0.5,1)
pipe_params['xgbr__colsample_bytree'] = optuna.distributions.CategoricalDistribution([0.3,0.5,0.8,1.0])
pipe_params['xgbr__reg_lambda'] = optuna.distributions.LogUniformDistribution(1e-4,1e0)  # L2 regularization 
